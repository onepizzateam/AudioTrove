...
for start in range(0, len(files), 200):
    ...
    stats = LocalExecutor(pipeline, checkpoint_path=f"{out}/checkpoint.db", num_workers=4).run(LocalAudioReader(files[start:start + 200], min_duration_seconds=0.0, max_duration_seconds=None), exporter)
    kept += stats["kept"]
...
with open("benchmarks/run_w4_result.json", "w") as f:
    json.dump(result, f)
